...
parser.add_argument('--fold', default=5, type=int, help='k fold number')
...
trainset = test301(split = 'Training', fold = opt.fold, transform=transform_train)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=opt.bs, shuffle=True, num_workers=0)
testset = test301(split = 'Testing', fold = opt.fold, transform=transform_test)
