...
class FERTRAIN(data.Dataset):

    def __init__(self, split='Training', fold = 1, transform=None):
        self.transform = transform
        self.split = split  # training set or test set
        self.fold = fold # the k-fold cross validation
        self.data = h5py.File('./data/fer_train_data.h5', 'r', driver='core')

        number = len(self.data['data_label']) #981
        sum_number = [0, 3995, 4431, 8528,15743, 20573, 23744, 28709]  # the sum of class number
        test_number = [1,1,1,1,1,1,1] # the
