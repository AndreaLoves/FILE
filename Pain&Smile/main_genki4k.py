...
import argparse
import utils
from data.genki import genki
from torch.autograd import Variable
# from models import *
from models.model import ER
import torch.nn as nn
# from csacnn import network
...
trainset = genki(split = 'Training', fold = opt.fold, transform=transform_train)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=opt.bs, shuffle=True, num_workers=0)
testset = genki(split = 'Testing', fold = opt.fold, transform=transform_test)
...
def test(epoch,net):
    ...
    if Test_acc > best_Test_acc:
        ...
        if not os.path.isdir(path):
            os.mkdir(path)
        torch.save(net.state_dict(), '/home/ouyangqun/emo_rec/emo_rec/dataset/params_gebki0308.pkl')
